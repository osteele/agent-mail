...
can read, since that is where `weft run` executes. Probing each agent:
...
Probing also showed that a child agent inherits its parent's
`CLAUDE_CODE_SESSION_ID` unchanged: a kimi launched from a Claude session sees
the Claude session's id.
...
- A session in an agent that exports no native id and is started outside the
  guard launcher still cannot be addressed. It gets the old broadcast.
...
- The weft half is not built. weft must capture the submitter id at `weft run`
  time, persist it on the job, and re-export it at notify time; it should read
  the first non-empty of an ordered, configurable list of env var names
  defaulting to `SESSION_ID_ENV_VARS`' order, and store the result as an opaque
  string. That keeps weft ignorant of both agent-mail and any specific agent.
  Until then every job notification is still a broadcast.
- Two repos now encode the same rule. A change to `SESSION_ID_ENV_VARS` needs a
  matching change to the launcher's unset list, and eventually to weft's default
  capture list.
